	...
	override function update(elapsed:Float)
	{
		...
		if(nextAccept <= 0)
		{
			...
			if(usesCheckbox)
			{
				...
				if(_vpad.buttonLeft.pressed || _vpad.buttonRight.pressed) {
					var pressed = (_vpad.buttonLeft.justPressed || _vpad.buttonRight.justPressed);
					if(holdTime > 0.5 || pressed) {
						if(pressed) {
							var add:Dynamic = null;
							if(curOption.type != 'string') {
								add = _vpad.buttonLeft.pressed ? -curOption.changeValue : curOption.changeValue;
							}
							...
							switch(curOption.type)
							{
								...
								case 'string':
									...
									if(_vpad.buttonLeft.justPressed) --num;
									else num++;
							...
							holdValue += curOption.scrollSpeed * elapsed * (_vpad.buttonLeft.pressed ? -1 : 1);
							if(holdValue < curOption.minValue) holdValue = curOption.minValue;
							else if (holdValue > curOption.maxValue) holdValue = curOption.maxValue;
				...
				} else if(_vpad.buttonLeft.justReleased || _vpad.buttonRight.justReleased) {
					clearHold();
				}
			}
