...
class NotesSubState extends MusicBeatSubstate
{
	...
	override function update(elapsed:Float) {
		if(changingNote) {
			if(holdTime < 0.5) {
				if(_vpad.buttonLeft.justPressed) {
					updateValue(-1);
					FlxG.sound.play(Paths.sound('scrollMenu'));
				} else if(_vpad.buttonRight.justPressed) {
					updateValue(1);
					FlxG.sound.play(Paths.sound('scrollMenu'));
				} else if(_vpad.buttonY.justPressed) {
					resetValue(curSelected, typeSelected);
					FlxG.sound.play(Paths.sound('scrollMenu'));
				}
				if(_vpad.buttonLeft.justReleased || _vpad.buttonRight.justReleased) {
					holdTime = 0;
				} else if(_vpad.buttonLeft.pressed || _vpad.buttonRight.pressed) {
					holdTime += elapsed;
				}
			} else {
				var add:Float = 90;
				...
				if(_vpad.buttonLeft.pressed) {
					updateValue(elapsed * -add);
				} else if(_vpad.buttonRight.pressed) {
					updateValue(elapsed * add);
				}
				if(_vpad.buttonLeft.justReleased || _vpad.buttonRight.justReleased) {
					FlxG.sound.play(Paths.sound('scrollMenu'));
			...
			if (_vpad.buttonUp.justPressed) {
				changeSelection(-1);
				FlxG.sound.play(Paths.sound('scrollMenu'));
			}
			if (_vpad.buttonDown.justPressed) {
				changeSelection(1);
				FlxG.sound.play(Paths.sound('scrollMenu'));
			}
			if (_vpad.buttonLeft.justPressed) {
			...
			if (_vpad.buttonRight.justPressed) {
				changeType(1);
				FlxG.sound.play(Paths.sound('scrollMenu'));
			}
			if(_vpad.buttonY.justPressed) {
				for (i in 0...3) {
					resetValue(curSelected, i);
				}
				FlxG.sound.play(Paths.sound('scrollMenu'));
			}
			if (_vpad.buttonA.justPressed && nextAccept <= 0) {
				FlxG.sound.play(Paths.sound('scrollMenu'));
				changingNote = true;
				holdTime = 0;
		...
		if (_vpad.buttonB.justPressed || (changingNote && _vpad.buttonA.justPressed)) {
			if(!changingNote) {
				close();
			} else {
				changeSelection();
